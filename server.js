...
http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && url.pathname === '/api/staff-dashboard') {
    ...
    return sendJson(res, 200, { assignments, stations, tasks, meta:{lastUpdated:new Date().toISOString(),stationCount:stations.length,source:'DEMO_MOCK',config:SYSTEM_CONFIG} });
  }
  if (req.method === 'GET' && url.pathname === '/api/stations') {
    const district = url.searchParams.get('district');
    const stations = district ? db.prepare('SELECT * FROM stations WHERE district = ? AND active = 1 ORDER BY name').all(district) : db.prepare('SELECT * FROM stations WHERE active = 1 ORDER BY district, name').all();
    return sendJson(res,200,{stations,stationCount:stations.length,lastUpdated:stations.map(item => item.observed_at).filter(Boolean).sort().at(-1) || null});
  }
  ...
  if (req.method === 'POST' && url.pathname === '/api/tasks') {
    const { username, district, worker, origin, destination, suggested } = await readBody(req);
  ...
  if (req.method === 'GET' && url.pathname === '/api/dashboard') {
    ...
    const stations = district === '全部行政區' ? db.prepare('SELECT * FROM stations WHERE active = 1').all() : db.prepare('SELECT * FROM stations WHERE district = ? AND active = 1').all(district);
    ...
    return sendJson(res, 200, { stations, tasks, assignments, staff, meta:{lastUpdated:new Date().toISOString(),stationCount:stations.length,source:'DEMO_MOCK',config:SYSTEM_CONFIG} });
  }
...
}).listen(port, () => console.log(`YouBike Demo: http://localhost:${port}`));
